...
export class AuthService {
    ...
    public async login(user: UserEntity): Promise<AuthDTO> {
        ...
        authDTO.token = jwt.sign({email: user.email}, secret, {expiresIn: expiresIn + expiresInUnit});
        authDTO.refreshToken = jwt.sign({email: user.email}, refreshSecret, {expiresIn: refreshTokenExpiresIn + expiresInUnit});
